...
(function () {
  ...
  function decorateButton(button) {
    ...
    if (button.dataset.upgradDoodleApplied === "true") return;
    ...
    button.querySelectorAll(
      ...
      ".upgrad-smo-icon, svg, i, " +

      "img:not(.upgrad-smo-doodle), " +

      "[class*='material-icon'], [class*='fa-'], " +

      "[class*='smo-icon'], [class*='button-icon']"

    ).forEach(function (oldIcon) {
    ...
    const observer = new MutationObserver((mutations) => {
      ...
      mutations.forEach((mutation) => {
        ...
        mutation.addedNodes.forEach((node) => {
          ...
          if (node.nodeType === Node.ELEMENT_NODE) applyDoodles(node);
